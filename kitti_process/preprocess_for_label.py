...
def preprocess(split_list, has_label=False, tag='train'):
    ...
    if has_label:
        det_list_file = REF_DIR + 'list_files/'
        if not os.path.exists(det_list_file):
            os.makedirs(det_list_file)
        est_list_file = REF_DIR + 'list_files/'
        if not os.path.exists(est_list_file):
            os.makedirs(est_list_file)
    ...
    for line in open(os.path.join(os.path.dirname(os.path.abspath(__file__)), split_list)):
        if line[-1] == '\n':
            line = line[:-1]
        image_id = int(line)
        ...
        points_save_path = REF_DIR + folder + '/cropped/'
    ...
    #
            ...
            xml_save_path = PRO_DIR + folder + '/VOCdevkit/Annotations/'
    ...
    out_path = '{}/annotations/'.format(PRO_DIR+folder)
