...
DATA_DIR = '/home/ubuntu/workstation/data/dataset/KITTI/'
PRO_DIR = '/home/ubuntu/workstation/data/dataset/kitti_fvnet2/projection/'
REF_DIR = '/home/ubuntu/workstation/data/dataset/kitti_fvnet2/refinement/'
...
def preprocess(split_list, tag='train'):
    ...
    lines = open(os.path.join(os.path.dirname(os.path.abspath(__file__)), split_list)).read().split('\n')[:-1]
    total_empty_rate = 0
    total_loss_rate = 0
    start = time.time()
    for line in lines:
        ...
        pts = pts[idx]
    #
        front_map, loss_rate, empty_rate = get_map_and_boxes2d(pts)
        total_empty_rate += empty_rate
        ...
        if not os.path.exists(front_png_save_path):
            os.makedirs(front_png_save_path)
        front_png_save_path = front_png_save_path + line + '.png'
        front_map_png.save(front_png_save_path, 'png')
    end = time.time()
    return total_loss_rate, total_empty_rate, end-start

def vis_pc_from_numpy(pts):
    '''
    Input(s): pts = 4d point cloud in numpy form. 4th dimension is the intensity
    Outut(s): visualization of the point cloud
    '''
...
def crop_camera(pts, P, Tr_velo_to_cam, R_cam_to_rect):
    ...
    idx2 = np.logical_and(np.logical_and(pts2d_normed[0, :] >= 0, pts2d_normed[0, :] <= 1242), # how to get the hard coded values?
...
def loop(num_point, index_h, index_w, height_ratio, dist_ratio, intensity):
    ...
    empty = np.mean(front_map == np.array([0,0,0]))
    return front_map, loss/all, empty
...
def get_map_and_boxes2d(points):
    ...
    # 192x384
    index_h = np.floor((zenith - Z_MIN) / z_range * HEIGHT).astype(int)
    index_w = np.floor((azimuth - A_MIN) / a_range * WIDTH).astype(int)
    
    front_map, loss_rate, empty_rate = loop(len(points), index_h, index_w, height_ratio, dist_ratio, intensity)
    front_map = FZ(front_map)
    front_map = (front_map * 255).astype(np.uint8)
    
    return front_map, loss_rate, empty_rate
...
if __name__ == '__main__':
    loss1, empty1, t1 = preprocess('image_sets/train.txt', tag='train')
    loss2, empty2, t2 = preprocess('image_sets/val.txt', tag='val')
    loss3, empty3, t3 = preprocess('image_sets/test.txt', tag='test')
    print("loss rate :%.5f"%((loss1+loss2+loss3)/14999))
    print("empty rate :%.5f"%((empty1+empty2+empty3)/14999))
